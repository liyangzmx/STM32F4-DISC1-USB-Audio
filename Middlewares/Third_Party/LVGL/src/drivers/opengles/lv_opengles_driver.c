...
void lv_opengles_init(void)
{
    ...
    /* 这个 OpenGL 后端的核心思路很简单：
     * 不管是显示一张纹理，还是填充一个纯色区域，最终都转成“画一个四边形（quad）”。
     * 初始化阶段只做一次共享 GPU 状态的准备，例如 blending、顶点缓冲、索引缓冲、shader 等。 */
    ...
    /* 这里只保留一个可重复写入的动态顶点缓冲。
     * 每次绘制前，把当前这次 quad 的顶点数据覆盖进去即可，
     * 没必要为每次 texture / fill 操作都重新创建一套独立 buffer。 */
...
void lv_opengles_render_display_texture(lv_display_t * display, bool h_flip, bool v_flip)
{
    ...
    /* display texture 这个接口默认就是“把整张 display 纹理完整画到目标区域”。
     * 如果 display 配了旋转，不是靠额外矩阵去算，而是直接重写 quad 的顶点顺序和 UV 布局。 */
    ...
    /* shader 里会读取一个 3x3 变换矩阵。
     * 这里的矩阵负责把“标准化的全屏 quad”映射到最终目标方向，
     * 同时处理水平翻转/垂直翻转。 */
                                        ...
                                        int32_t disp_w, int32_t disp_h, const lv_area_t * texture_clip_area, bool h_flip, bool v_flip, lv_color_t fill_color)
{
    LV_PROFILER_DRAW_BEGIN;
    /* 先求一次交集。
     * 只有目标区域和裁剪区域真正相交，才有必要继续生成顶点和发起绘制。 */
    ...
    /* 这里把 LVGL 使用的像素坐标，换算成 shader 最终要用的缩放和位移参数。
     * shader 里先假设有一个标准 quad，再通过这组参数把它放到 framebuffer 的正确位置。 */
    ...
    if(texture != 0) {
        /* 目标区域被裁剪后，源纹理的采样范围也必须同步收缩。
         * 所以这里要把“屏幕上的裁剪结果”反推回纹理坐标系中的 UV 区间，
         * 这样 shader 采样时才只会读到真正可见的那一部分纹理内容。 */
        ...
        /* 顶点缓冲的布局是 `[x, y, u, v]`，四个顶点一共 16 个 float。
         * 其中位置部分仍然使用标准 quad 的四个角；
         * 真正跟裁剪、翻转相关的差异，主要通过 UV 坐标来表达。 */
...
static void lv_opengles_enable_blending(void)
{
    /* 混合方式使用的是适合本后端输出格式的 alpha blending 组合。
     * 开启后，后画上去的纹理/颜色块就可以按照透明度和已有内容正确混合。 */
...
static void lv_opengles_vertex_array_add_buffer(void)
{
    ...
    /* 约定顶点属性：
     * attribute 0 -> 顶点位置 (x, y)
     * attribute 1 -> 纹理坐标 (u, v)
     * 两者都按 float 打包在同一个顶点缓冲里。 */
...
static unsigned int lv_opengles_shader_manager_init(void)
{
    ...
    /* 不同平台/驱动支持的 GLSL ES 版本可能不一样。
     * 这里会按顺序尝试可用版本，直到当前运行环境能够成功编译并链接出一套 program。 */
...
static void lv_opengles_shader_deinit(void)
{
    ...
    /* program 的生命周期由 shader manager 统一管理，这里只需要销毁 manager。 */
    lv_opengl_shader_manager_deinit(&shader_manager);
    shader_id = 0;
}
...
static int lv_opengles_shader_get_uniform_location(const char * name)
{
    ...
    /* uniform location 查一次就缓存起来。
     * 否则每一帧、每一次 draw 都去 `glGetUniformLocation`，开销会比较浪费。 */
...
static void lv_opengles_shader_set_uniformmatrix3fv(const char * name, int count, const float * values)
{
    LV_PROFILER_DRAW_BEGIN;
    /*
     * GLES 2.0 不支持通过 `glUniformMatrix3fv` 在上传时做转置，
     * 因此这里传入的已经是预先转置好的矩阵。
     * 参考: https://registry.khronos.org/OpenGL/specs/es/2.0/es_full_spec_2.0.pdf 第 47 页
     */
...
static void lv_opengles_render_draw(void)
{
    LV_PROFILER_DRAW_BEGIN;
    /* 真正发起绘制的地方就在这里。
     * 共享同一套 shader / VAO / index buffer，用两片三角形拼成一个 quad。 */
    lv_opengles_shader_bind();
    lv_opengles_vertex_array_bind();
...
/**
 * 从 `lv_math.h` 里的 `lv_map` 思路改写而来，这里专门处理 float 映射。
 */
static float lv_opengles_map_float(float x, float min_in, float max_in, float min_out, float max_out)
{
    ...
    /**
     * 理论上的映射公式是：
     *   ((x - min_in) * delta_out) / delta_in + min_out
     * 这里保持同样的数学含义，只是把计算顺序整理得更稳定一些。 
     */
                                   ...
                                   float clip_y2)
{
    ...
    LV_UNUSED(v_flip);
#endif
    /* 旋转不是丢给 shader 动态算，而是在 CPU 侧生成顶点数据时直接改掉四个角的顺序和 UV。
     * 这样 shader 路径会更轻，常见的 display rotation 也更直接。 */
    switch(rotation) {
